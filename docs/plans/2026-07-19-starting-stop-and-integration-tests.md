...
- [x] **13. LM Studio lifecycle** — `internal/launcher/integration_lmstudio_test.go` (new, build-tagged) — ✅ DONE (2026-07-19)
  - NOTES (2026-07-19): "same shape" resolved against Item 12's file as implemented, with the LM Studio deltas recorded here. The stop step is the unified `Stop(addr)` per Item 12's precedent (it runs the `lms server stop` hook inside the real TDD §6.5 sequence); its assertion checks the identified backend, not a PID — LM Studio implements no PIDTracker, so cleanup also cannot reuse `killServerOnCleanup` and is instead best-effort `Stop(addr)` plus a bare `TryStop` backstop for a not-yet-identifiable half-started server. Addr binding verified per Item 12's convention: `TryStart` forwards only the port (`--port`) and LM Studio picks the bind interface, so `freePort` avoids collisions but cannot isolate from the user's own server — the suite moves the single app server, which is exactly the accepted interference the mandated file comment documents. Model steps gate on `INTEGRATION_MODEL_LMSTUDIO` (Item 15's env-var family, model already downloaded — LoadModel does not download); unload uses the identifier `/v1/models` reports (tolerating `key:2` instance suffixes) and polls until the listing drops it, per Item 12's assert-the-effect convention. Layer-2 CHANGELOG/TDD debt stays with Item 15 per Item 10's note.
...
- [x] **14. Makefile targets** — `Makefile` — ✅ DONE (2026-07-19)
...
- [x] **15. Layer-2 docs pass** — `llama-launcher.TDD.md`, `README.md`, `CHANGELOG.md` — ✅ DONE (2026-07-19)
  - NOTES (2026-07-19): the cited line numbers had drifted (TDD 844/866 are 849/871 today; README 512 is the 512–514 Building block) — same content, all occurrences fixed, including TDD §14's "run `make install` after changing code" step, which now says `make build` + Homebrew. Beyond the item's literal three bullets, the pass carries the debts Items 10–14 deferred here: TDD gains §12.5 (integration-suite table, conventions, env-var table) beneath the first bullet's two-layer §12 intro, and the new targets in §13's command block; the CHANGELOG "Added" entry covers the suites *and* the `make test`/`test-integration`/`test-all` targets in one entry; README's Building section documents the targets, the host-only rule, and the `INTEGRATION_MODEL_*` vars. A separate CHANGELOG "Fixed" bullet records the `make install` doc-bug correction (docs only). All entries sit under `## Unreleased` per Item 8 precedent (Item 18 gates the version header). The intro's decision 8 says "(Item 16)" for the install-doc bug — a slip; this item's text owns it and it is fixed here.
