...
- **llamacpp** — exported into the launched server's environment as `LLAMA_API_KEY`, so llama-server rejects client requests lacking `Authorization: Bearer <key>` (`/health` stays exempt). The key is deliberately kept off the process argv, where any local user could read it out of `ps`. llama-server reads the env var only when no `--api-key` flag is present (llama.cpp `common/arg.cpp` is the authority), so an `extra_args` `--api-key` override still wins — and such a literal override *is* argv-visible, which is why display surfaces still redact it.
