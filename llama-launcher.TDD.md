...
When a server the launcher started itself misses its startup wait (`ErrStartupTimeout`, [§6.2](#62-starting-an-llm-server)), the menu does not show the timeout as an error. The progress popup is replaced by a `Still loading <profile>… m:ss` popup — the time since the load began, redrawn every second — with the hint `Esc to return`, which polls the server once a second. When the server turns healthy the popup closes and the confirmation above is printed. When the server is gone — not healthy, not Starting and its PID no longer alive — on two polls in a row, the error popup shows the original timeout text plus `The server is no longer running.` Esc (or `q`, or Ctrl+C) returns to the menu with the server left loading, where it shows as `starting…`. Only a managed timeout opens this popup; the external auto-start's timeout, and every error when stdin is not a terminal, surface as before.
