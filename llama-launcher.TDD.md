...
| `llama-launcher status [--json]` | Print one row per running LLM Server instance (address, backend, active Profile/Model, PID if known, uptime). Exit code 0 if any running, 1 if all stopped. With `--json`, emits a JSON array with one entry per running instance plus one `running: false` entry per enabled backend with no running instance (`backend`, `running`, `address`, `active_profile`, `active_model`, `pid`, `uptime_seconds`); same exit code semantics. |
