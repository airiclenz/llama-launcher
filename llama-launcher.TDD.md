...
| `backend.go` | `LLMServer` and `ManagedLLMServer` interface definitions (see [§5.3](#53-llmserver-interface)), `ResolvedProfile` struct, `ProfileParamSpec` display specs (the shared `spec*` values plus the `intParamSpec`/`boolParamSpec` constructors), LLM Server registry (register/get), `applyAPIKeys` (pushes per-server API keys from the config onto backends implementing the package-private `apiKeyConfigurable`). |
