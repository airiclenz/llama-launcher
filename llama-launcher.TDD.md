...
After selecting a Profile, the launcher shows a step-by-step progress popup that updates in place as each lifecycle stage completes, then prints a confirmation and exits. With `auto_close: false` the menu comes back instead, and the confirmation is skipped: the popup gains a final `Refreshing menu` step and stays on screen until the menu repaints over it, so the discovery pass before that repaint — seconds on a machine saturated by the model it just loaded — never shows a blank screen:
...
When a server the launcher started itself misses its startup wait (`ErrStartupTimeout`, [§6.2](#62-starting-an-llm-server)), the menu does not show the timeout as an error. The progress popup is replaced by a `Still loading <profile>… m:ss` popup — the time since the load began, redrawn every second — with the hint `Esc to return`, which polls the server once a second. When the server turns healthy the popup closes and the confirmation above is printed (with `auto_close: false` the popup stays up for the menu to repaint over, as on a normal load). When the server is gone — not healthy, not Starting and its PID no longer alive — on two polls in a row, the error popup shows the original timeout text plus `The server is no longer running.` Esc (or `q`, or Ctrl+C) returns to the menu with the server left loading, where it shows as `starting…`. Only a managed timeout opens this popup; the external auto-start's timeout, and every error when stdin is not a terminal, surface as before.
...
| `launcher/launcher_test.go` | Facade tests in an external test package (`package launcher_test`), so they reach the launcher only through its exported surface, exactly as a client module does: config load with the warning sink (and merged `context_size` off the resolved profile), the two sentinel-error paths, httptest-backed discovery, and the drift notice threaded end-to-end through a plain `LoadProfile` on the ADR-0007 idempotent path with nothing reaching stderr, and `Unload` against a 401 listener at an unconfigured address (`ErrNotRunning`). httptest and temp directories only — no real processes; the one real process is `launcher_unix_test.go`'s. |
| `launcher/launcher_unix_test.go` | `//go:build !windows` facade tests (`package launcher_test`) that need a real listener to signal: `TestFacadeAuthRefusingHelperProcess` re-executes the test binary in its own session (`Setsid`) serving 401 until signalled, and the `TestStop_*AuthRefusingListener*` pair pins ADR-0010's configured-address scope — `Stop` returns `ErrNotRunning` and leaves the child serving at an address no loaded config names, and stops it (`Backend ""`, the child's PID, child reaped) once `LoadConfig` points a backend there. |
...
| `process_unix.go` | Process control on darwin and linux, behind `//go:build unix` — the six functions that hold every unix-only primitive the server lifecycle needs: `listProcesses()` (the `ps -A -ww -o pid=,pgid=,command=` process table behind the loading-Splash match, with each session leader's command line replaced by its true argv from `procArgv`, [ADR-0015](docs/adr/0015-a-loading-splash-is-found-by-its-process.md)), `detachedSysProcAttr()` (`Setsid`, so a spawned server outlives the launcher and is stoppable as a process group), `signalPID`/`signalGroup` (today's `syscall.Kill(pid, …)` / `syscall.Kill(-pid, …)` verbatim), `requireProcessControl()` and `requireProcessStop()`, which both return nil here ([§16.6](#166-platform-contract)). |
| `process_windows.go` | The same six signatures under `//go:build windows`, each refusing instead of acting: `detachedSysProcAttr` returns nil (no spawn ever reaches it), and the other five return an error wrapping `ErrUnsupported` — so a loading Splash is never found on windows. Windows has no session to detach into and no process group to signal, so the file is what lets the package build — and actuate over HTTP — there ([§16.6](#166-platform-contract)). A real implementation (Job objects, `taskkill`) would land here additively. |
...
| `discovery.go` | `RunningInstance` type and `DiscoverRunningInstances(cfg)` — probes every (backend, address) pair derivable from the config in parallel, returns the reachable set with the loaded Model. Optional runtime details (PID via `lsof`, start time via `ps -o lstart=`, log path via the deterministic naming convention) are populated lazily by `fillRuntimeDetails`. An address where every probing backend answers 401/403 becomes one `AuthFailed` row (`authFailedRows`); `authRefusalAt` applies the same rule to one address for `load`. `applyConfiguredTargets` (called by `LoadConfigNotify`) keeps the process-global address → backend-names snapshot of the last loaded config that `configuredBackendsAt` hands `identifyBackend`'s 401/403 pass (§6.5). `instancesSignature` condenses a discovery result into a comparable string (backend, address, loaded model, Starting and AuthFailed per instance) used by the menu to detect background state changes between refresh ticks. `boundModelID` caps the stored `ActiveModel` at `maxModelIDBytes` (512) on a rune boundary, appending `…`; Profile matching runs before it, on the whole id. |
...
| `progress.go` | Step-by-step progress feedback for lifecycle operations. `ProgressFunc` callback type, `progressTracker` (TUI popup that updates in place; a 1 s ticker goroutine redraws it so the active step shows its elapsed time as `m:ss`, and `Close()` stops the ticker before the caller clears the screen), `newCLIProgress` (plain text fallback), `printStopSteps` (after-the-fact CLI rendering of the steps a unified `Stop`/`Unload` returns in its `StopResult`). Home of the sibling `NoticeFunc` sink and its `reportNotice` helper (§16.3) — user-facing notices (config warnings, the ADR-0007 drift notice) go to the UI layer through it instead of straight to stderr; a nil sink discards, exactly like `reportStep`. |
...
| `discovery_test.go` | httptest-based tests for `DiscoverRunningInstances` (reachable / unreachable, hostile model names sanitised, over-long ids bounded), `boundModelID`, `LlamaCpp.ListRunningModels` (including the oversized-body cap), `LlamaCpp.QueryLiveParams` (`/props` populated and 404 fallback), and `findManagedLogFile` (most-recent picker by lexicographic timestamp). |
| `menu_test.go` | Tests for `parseChoice`, `formatUptime`, `profileDisplayName`, the Profile-list context-size column (`formatContextSize`'s number format plus direct coverage of `buildProfileItems` and `buildSimpleProfileLines`: merged values, right-alignment, the blank Ollama cell, the presence gate's byte-identical pre-column shape, the single-enabled-server case, and the still-rightmost `★`), and `formatProfileParams` (`TestFormatProfileParams_LMStudio` asserts LM Studio profiles omit GPU offload — it is not part of the load request — while showing the params the load request does send; `TestFormatProfileParams_RendersBackendOwnedSpec` registers a stub backend and asserts the pop-up renders exactly its `ParamSpecs`, in order, proving a new backend needs no menu edit; API-key redaction and the empty Ollama spec are covered separately). `TestWaitStillLoading_*` drive the still-loading loop with scripted probes and keys (healthy on the third poll, gone on two polls in a row, Esc/Ctrl+C/q leaving the server), and `TestDoLoadProfile_RoutesStartupTimeout` pins that only a terminal-mode managed timeout reaches the waiter — the external timeout and non-terminal errors pass through unchanged. |
| `helpers_test.go` | Shared test helpers: `addrFromURL` for extracting `host:port` from httptest server URLs, `deadAddr` for a loopback address nothing listens on, and `pinConfiguredTargets` for setting the process-global configured-address snapshot (§6.5) from a `Config` and restoring it at cleanup — its callers are non-parallel top-level tests. |
...
9. Wait for backend health check to succeed. On `load` — and so on `start --profile`, which runs through `load` — the wait (`waitForStartup`) lasts as long as the spawned server makes startup progress: its log file grew since the previous poll, or its `StartupProber` answers that it is still loading (llamacpp `/health` 503, Splash `/ready` 503; a Splash `LoadingPID` hit alone is not progress). It gives up after `startup_stall_timeout` (default 30 s) without progress, or after `startup_max_wait` (default 10 min) in total. On timeout the spawned process is **left running** — a large Model on a cold disk can legitimately need longer — and the error names its PID and log path with recovery guidance: watch the log and retry once healthy, or `llama-launcher stop <backend>` (ADR-0010 made stopping a Starting server the launcher's own job, so the guidance no longer points at a manual `kill`). A plain retry while it is still loading hits the step-1 refusal; a retry after it turns healthy is the idempotent no-op ([ADR-0007](docs/adr/0007-profile-activation-idempotency.md)). On `load` the wait also watches the spawned process's exit: when it ends mid-wait the load returns at the next poll gap instead of waiting out the window — a non-zero exit code as a crash error with the redacted log tail, anything else (status 0, a signal, 128+SIGTERM/SIGINT, an unreadable status — what a `stop` of the loading server produces) wrapping `ErrLoadCanceled` (§16.2).
...
| `TestLoadProfile_StartupTimeoutIsErrStartupTimeout` | The **real** activation wait loop against an httptest stand-in that answers 503 forever: the error wraps `ErrStartupTimeout` (so a client recognises the outcome without matching on text), still names the PID, the log path and the "left running" guidance verbatim, and the timed-out server is not stopped. The health wait is the only real operation: `realWaitOps` overrides just `waitHealthy` (production `waitForStartup`, with the stall window and cap shortened — the production 30 s and 10 min would stall the suite) and embeds `fakeOps` for the rest, so start, stop, discovery and the probes stay in memory; the PID and log path the message names are the fake instance's, and nothing is forked or signalled. The error also carries that address, PID and log path for in-package callers: `errors.As` reads them off the `startupTimeout`. |
| `TestWaitForStartup_LogGrowthKeepsWaiting`, `TestWaitForStartup_StallTimesOut`, `TestWaitForStartup_StartingUpIsProgress`, `TestWaitForStartup_ExitEndsWait` | The managed load's progress-aware wait against httptest stand-ins: a log appended every 300 ms keeps a 1 s stall window alive while `/health` answers 500 until the server turns healthy; a static log with no Starting answer fails with "no startup progress for" after about one stall window; a llamacpp 503 (`StartingUp`) with a static log runs to the cap ("within"); a closed exit channel ends the wait with `errServerExited`. |
...
`helpers_test.go` provides `addrFromURL(t, rawURL) string`, which parses an `httptest.NewServer` URL and returns the `host:port` portion for passing to backend methods that expect an `addr` string. `pinConfiguredTargets(t, cfg)` sets the configured-address snapshot a 401/403 stop is scoped to (§6.5) as a `LoadConfig` of `cfg` would, and restores the previous snapshot in `t.Cleanup`; the snapshot is process-global, so its callers are non-parallel top-level tests, and an unconfigured leg passes `&Config{}` rather than assuming an earlier test left the set empty.
...
The platform knowledge lives in per-OS build-tagged files, never in a `runtime.GOOS` branch inside a shared path: `process_unix.go` / `process_windows.go` (`detachedSysProcAttr`, `signalPID`, `signalGroup`, `requireProcessControl`, `requireProcessStop`, `listProcesses`), `proc_argv_darwin.go` / `proc_argv_linux.go` / `proc_argv_other.go` (`procArgv`, the true argv behind `listProcesses`), `proc_identity_darwin.go` / `proc_identity_linux.go` / `proc_identity_windows.go` (`processIdentity`, the start-time check before each stop signal), `config_trust_unix.go` / `config_trust_windows.go` (`configTrusted`, the `api_key_cmd` ownership gate) and `ui_poll_darwin.go` / `ui_poll_linux.go` / `ui_poll_windows.go` (`pollStdin`, `requireInteractiveMenu`). One `runtime.GOOS` read in a shared path is deliberate: `menu.go`'s `editConfigGOOS` picks what "Edit config" runs. That verb needs no platform primitive, only a different program (`open` or the user's editor), so both branches compile everywhere and a test can drive either on any host. The start guards (`requireProcessControl`, `requireInteractiveMenu`) exist so a refusal happens *before* the irreversible step — forking a child no stop path could reach would be worse than not starting one. The stop guard, `requireProcessStop`, names its refusal *after* the stop mechanisms ran rather than before: a backend's native stop hook (LM Studio's `lms server stop`) needs no process control and must still get its chance. Because Go's implicit filename constraints bind that set, the package's platform vocabulary is now exactly darwin, linux and windows; other unixes (freebsd, which used to compile the BSD poll) are outside the contract.

**What `errors.Is(err, ErrUnsupported)` actually holds for.** Each windows stub returns an error wrapping the sentinel, and each of the four windows refusal paths hands it back wrapped. All four hold:
...
- **Holds — the stop verbs.** `IsProcessAlive` is `signalPID(pid, 0) == nil`, always false on windows, so `terminatePID` is never reached; `Stop` / `Unload` fall through to the backend's own stop hook. LM Studio's `lms server stop` works, so the stop genuinely succeeds there. llamacpp's, Splash's and Ollama's hooks are no-ops (§6.5), so the server stays reachable, and `stopServerAt` then asks `requireProcessStop()` and wraps its refusal: `server at <addr> is still reachable and could not be stopped: stopping a server process: operation not supported on this platform` (CLI exit 3). The same error reaches a client through `Stop`, an `Unload` that reduces to a stop on a managed backend, and `LoadProfile`'s stop step (the auto-stop sweep and a managed restart, both via `StopInstance`). The one exception is a stop hook that itself failed: that error comes first and names the hook (`… stop hook failed: …`) without the sentinel.

So `ErrUnsupported` is precise at the **seam** and at every windows refusal path the launcher has. ADR-0012 was amended on 2026-07-29 to describe the split the code then implemented — its windows bullet had claimed the sentinel for every kill-by-PID stop path, which the code did not yet do — and recorded the external auto-start and the stop verbs as a known gap in its own precision. The external auto-start was closed on 2026-09-24 (ADR-0012's second amendment) and the stop verbs the same day (its third amendment), with no change to darwin or linux behaviour: there the stop guard permits, and every stop outcome and message is byte-identical. The stop wrap is proved on a unix host by substituting a refusing `processStopGuard` (`TestStop_RefusingStopSeamWrapsErrUnsupported`, §12.2).
