...
Per-server API keys do not appear in any interface signature: several call paths (`identifyBackend`, `WaitForHealth`, instance stop/unload) have no `*Config` in scope. Instead each backend struct holds an unexported `apiKey` field, set by `applyAPIKeys` at the end of `LoadConfig` (and thus refreshed on every `Reload`), and attaches it as a `Bearer` header via the helpers in `backend_http.go`. Because `LoadConfig` can run concurrently with in-flight discovery probes (and with another `LoadConfig`), the field is guarded by a per-backend `sync.RWMutex`: `setAPIKey` writes under the write lock and every read goes through a `getAPIKey` accessor under the read lock, so the config-load goroutine and the parallel `HealthCheck`/`ListRunningModels` probe goroutines never touch it unsynchronised. Health-check discrimination is unaffected: a key-protected llama-server still answers its auth-exempt `/health`, and it 401s LM Studio's `/v1/models` probe (a correct rejection); LM Studio's own probes carry the key so they keep working when its token requirement is enabled.
...
It probes them all in parallel with `LLMServer.HealthCheck`. For every reachable address it then asks the backend for the currently loaded Model via `ModelLister.ListRunningModels` (`/v1/models` for llamacpp / LM Studio, `/api/ps` for Ollama). Live server parameters are *not* queried during discovery; `LiveParamsQuerier.QueryLiveParams` (llamacpp `/props`) is used only by the `LoadProfile` drift check (§9 / ADR-0007).
...
| `TestDiscoverRunningInstances_*` | Discovery returns the empty set when nothing listens; an httptest llama-server stand-in is found with `ActiveModel` populated from `/v1/models`. |
