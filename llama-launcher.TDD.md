...
| `progress.go` | Step-by-step progress feedback for lifecycle operations. `ProgressFunc` callback type, `progressTracker` (TUI popup that updates in place), `newCLIProgress` (plain text fallback), `printStopSteps` (after-the-fact CLI rendering of the steps a unified `Stop`/`Unload` returns in its `StopResult`). |
