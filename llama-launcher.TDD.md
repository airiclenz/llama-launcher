...
- **[docs/adr/](docs/adr/)** — numbered Architectural Decision Records (ADRs 0001–0011) that pin down behaviour.
...
| `launcher/launcher.go` | The public library facade ([§16](#16-public-library-facade), [ADR-0011](docs/adr/0011-public-library-facade.md)) — the only package outside `internal/` another module can import. Type aliases (`Config`, `Profile`, `ProfileParams`, `ResolvedProfile`, `RunningInstance`, `StopResult`, `ProgressFunc`, `NoticeFunc`), the two sentinel errors re-exported by value so `errors.Is` matches across the boundary (`ErrConfigNotFound`, `ErrNotRunning`), and seven one-line wrapper functions. Wrappers contain zero logic (the ADR-0009 `realOps` discipline applied to the facade): `LoadConfig` and `LoadProfile` delegate to the notice-taking `LoadConfigNotify` / `LoadProfileNotify`, the rest delegate to their internal namesake. |
| `launcher/doc.go` | Package documentation carrying the facade's contract (§16.2): the documented surface is the API, notices are callbacks, verbs block and cancellation is `Stop(addr)`, one `Config` per process — plus the scope note (local machine only, remote control belongs to the MCP adapter) and a godoc usage example. |
...
| `server_test.go` | Tests for `IsProcessAlive` (including PID 0 guard), `readLastLines`, `RunningInstance` methods (`Addr`, `Uptime`), `paramDrift`, `liveParamDrift`, `shouldCrossServerUnload`, `WaitForHealth`, the stop decision layer (`StopInstance`, `identifyBackend`, `terminatePID`), and the `LoadProfile` orchestration ([ADR-0009](docs/adr/0009-activation-operations-seam.md)): fake-`activationOps`-driven tests for the idempotent no-op, `--restart`, the `auto_stop_server`/`auto_unload` matrix, and the external swap/connect fork, plus httptest-backed tests (real probes, recorded stops via `stopRecordingOps`) for the shared-address foreign-occupant stop and the still-starting-up double-spawn refusal. The unified `Stop`/`Unload` entry points are covered through the same seam (`steppedOps`): managed backend → server stop, external backend → API unload with the server left running, step collection into the `StopResult`, and error paths. |
...
Seventeen symbols, all in `launcher/launcher.go`:
...
`launcher/launcher_test.go` is an **external** test package (`package launcher_test`): it may only reach the launcher through the exported surface, so it proves the client's view compiles and behaves. It covers config load with the warning sink and the merged `context_size` off the resolved profile, both sentinel-error paths (`errors.Is` across the module boundary), httptest-backed discovery, and the drift notice threaded end-to-end — a plain `LoadProfile` on the ADR-0007 idempotent path against an httptest llama-server stand-in, delivering exactly one notice to the sink with nothing reaching stderr. No real processes: httptest and temp directories only, so it runs under plain `make test` (§12).
